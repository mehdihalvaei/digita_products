...
class User(AbstractBaseUser,PermissionsMixin):
    username = models.CharField(_('username'),max_length=32,unique=True,
                                help_text=_(
                                    'Required 30 Characters of fewer starting with a letter.Letters , digits and underscore only.'
                                ),
                                validators=[
                                    validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                                                              _('Enter a valid username starting with a-z.'
                                                                'This value many contain only letters,numbers'
    ...
    phone_number = models.BigIntegerField(_('mobile number'),unique= True,null=True,blank=True,
                                          ...
                                          error_messages={
                                              'unique':_("A user with the mobile number already exists"),
                                          }
                                          )
    is_staff = models.BooleanField(_('staff status'), default=False,
                                   help_text=_('Designates whether the user can log into this admin site.'))
    is_active = models.BooleanField(_('active'), default=True,
                                    help_text=_('Designates whther this user should be treated as active.'
                                                'Uselect this instead of deleting accounts.'))
...
class Device(models.Model):
    ...
    user = models.ForeignKey(User,related_name='divices',on_delete=models.CASCADE)
    ...
    device_type = models.PositiveSmallIntegerField(choices=DEVICE_TYP_CHOICES,default=WEB)
